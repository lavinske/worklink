...
if ( isset($_POST['username']) && isset($_POST['password']) ) {
    ...
    $sql_check = "SELECT id, username, status
                  FROM freelancer 
                  WHERE 
                       username=? 
                       AND 
    ...
    if ( $check_log->num_rows == 1 ) {
        ...
        $key = sha1($token . $randomNumber);
            $query = "UPDATE freelancer SET enckey = '$token' WHERE username = '$username'";
    mysqli_query($dbconnect, $query);
    if($_POST['rememe']) { setcookie("token", $token, time()+3600*24*365*10, "/"); 
    setcookie("loggeduser", base64_encode(base64_encode($username)), time()+3600*24*365*10, "/"); } 
    else { setcookie("token", $token, time()+3600, "/");  
    setcookie("loggeduser", base64_encode(base64_encode($username)), time()+3600, "/"); }
        }
