<?php include('../../app-assets/inc/freelancerheader.php'); 
  ...
  if(isset($_GET['id']) && isset($_POST['kirimfile'])){
    $errors = array();
    $id = $_GET['id'];

    $dirUpload = "../../kerjaan/";

    $namaFile = $_FILES['uploadHSL']['name'];
    $namaSementara = $_FILES['uploadHSL']['tmp_name'];
    $ext=substr($namaFile,strrpos($namaFile,'.')+1);
    if($ext != "zip" && $ext != "rar" && $ext != "7z" && $ext != "tar") array_push($errors, "not archive");
$frname=date("Y-m-d-H-i-s")."-";
$frname.="KERJAAN_"."-".$uname."-";
$frname.=uniqid().".".$ext;
if(count($errors) == 0){
  $terupload = move_uploaded_file($namaSementara, $dirUpload.$frname);
  $query = "UPDATE work SET filename='$frname', progress='75' WHERE projectID = '$id' AND handled='$uname'";
...
$sql_check = "SELECT *
    ...
    $check_log->bind_param('ss', $uname, $projectid);
    ...
    if (!$check_log->num_rows == 1 )
      header('location:http://'.$_SERVER['HTTP_HOST'].$linkprefix.'/user/home');
    $check_log->close();

    $getJobTLQ = "SELECT * FROM work JOIN company on cID = company.username WHERE projectid = '$projectid'";
    $resultTL = mysqli_query($dbconnect, $getJobTLQ);
    $rowTL = mysqli_fetch_assoc($resultTL);
...
<div class="row">
  <div class="col-sm-12">
    <div class="content-header"><?= $rowTL['name']; ?></div>
    <p class="content-sub-header">from <a href="mailto:<?= $rowTL['email']; ?>"><?= $rowTL['companyname']; ?></a> - for <?= $fmt->formatCurrency($rowTL['amount'], "Rp. "); ?></p>
  </div>
</div>
<div class="card">
    ...
    <div class="card-body">
      <section class="cd-horizontal-timeline loaded">
        <div class="timeline">
          <div class="events-wrapper">
            <div class="events" style="width: 100%;">
              <ol>
                <li><a style="left:5%;" class="<?php echo $rowTL['progress'] == 25 ? "selected" : ""?> " disabled>25%</a></li>
                <li><a style="left:25%;" class="<?php echo $rowTL['progress'] == 50 ? "selected" : ""?> " disabled>50%</a></li>
                <li><a style="left:70%;" class="<?php echo $rowTL['progress'] == 75 ? "selected" : ""?> "disabled>75%</a></li>
                <li><a style="left:90%;" class="<?php echo $rowTL['progress'] == 100 ? "selected" : ""?> " disabled>100%</a></li>
              </ol>
        ...
        <div class="events-content">
          <ol>
            <li class="<?php echo $rowTL['progress'] == 25 ? "selected" : "hidden"?> " data-date="25/01/1900">
              <h2 class="text-bold-500">Waiting for company selection</h2>
              <h4 class="text-bold-500 mb-2">25%</h4>
              <p>
                Sit tight. The company's admins are selecting.
              </p>
            </li>

            <li class="<?php echo $rowTL['progress'] == 50 ? "selected" : "hidden"?> " data-date="25/06/1900">
              <h2 class="text-bold-500">In Progress.</h2>
              <h4 class="text-bold-500 mb-2">50%</h4>
              <p>
                When you are ready and all the requirement fullfiled. Please upload your work below :

                  <form action="" method="POST" enctype="multipart/form-data">
                       <div class="input-group mb-3">
                      <div class="custom-file">
                        <input type="file" name="uploadHSL" accept=".zip,.rar,.7zip" class="custom-file-input" id="inputGroupFile02">
                        <label class="custom-file-label" for="inputGroupFile02" aria-describedby="inputGroupFileAddon02">Choose file</label>
                      </div>
                      <div class="input-group-append">
                        <button type="submit" name="kirimfile" class="input-group-text" id="inputGroupFileAddon02">Upload</button>
                      </div>       
                      </div>           
                    </form>
            ...
            <li class="<?php echo $rowTL['progress'] == 75 ? "selected" : "hidden"?>" data-date="25/01/1902">
              <h2 class="text-bold-500">Waiting for Company Approval</h2>
              <h4 class="text-bold-500 mb-2">75%</h4>
              <p>
                Company's admins are validating and verifying your work. Finding bugs and other problems to be fixed, if there's a problem, your progress will go down to 50%, and Company Admin will contact you ASAP.
              </p>
            </li>

            <li class="<?php echo $rowTL['progress'] == 100 ? "selected" : "hidden"?> " data-date="25/16/1902">
              <h2 class="text-bold-500">Finished! Payment Sent</h2>
              <h4 class="text-bold-500 mb-2">100%</h4>
              <p>
                The amount of saldo in your account are increasing, pending saldo are moved to active saldo!  
              </p>
            </li>
          </ol>
        </div>
