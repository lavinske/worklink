...
if(isset($_POST['wd'])){
  ...
  $user_check_query = "SELECT saldo FROM freelancer WHERE username='$uname'";
  $result = mysqli_query($dbconnect, $user_check_query);
  $user = mysqli_fetch_assoc($result);
  
  if ($user) { // if user exists
    if ($usaldo - $nominal < 0) {
      array_push($errors, "Nominal Kurang");
    }
  }

  if (count($errors) == 0) {
    $temp_saldo = $usaldo-$nominal;
    $temp_pending = $pendusaldo+$nominal;
    $query = "UPDATE freelancer SET saldo='$temp_saldo', pendsaldo='$temp_pending' WHERE username='$uname' AND enckey='$token'";
    $sql = mysqli_query($dbconnect, $query);
    $query1 = "INSERT INTO withdrawuser (username, norek, namabank, cabangbank, namarekening, nominal) 
              VALUES('$uname', '$norek', '$namabank', '$cabangbank', '$namarek', '$nominal')";
    $sql1 = mysqli_query($dbconnect, $query1);
    $beritamutasi = "Request Tarik ke Rekening ".$namabank." a/n ".$namarek." (".$norek.").";
    $nominal*=-1;
    $query2 = "INSERT INTO mutasiuser (username, berita, nominal) 
              VALUES('$uname', '$beritamutasi', '$nominal')";
    $sql2 = mysqli_query($dbconnect, $query2);

if ($dbconnect->query($sql) === TRUE && $dbconnect->query($sql1) === TRUE) {
        ...
        <div class="card-body pt-2 pb-0">
          <div class="media">
            <div class="media-body white text-left">
              <h3 class="font-large-1 mb-0"><?php echo $fmt->formatCurrency($usaldo, "Rp. "); ?></h3>
              <span>Saldo</span>
            </div>
    ...
    <div class="card gradient-ibiza-sunset"> <!-- bisa green-tea sama pomegranate -->
      <div class="card-content">
        <div class="card-body pt-2 pb-0">
          <div class="media">
            <div class="media-body white text-left">
              <h3 class="font-large-1 mb-0"><?php echo $fmt->formatCurrency($pendusaldo, "Rp. "); ?></h3>
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header">
          <h4 class="card-title text-center font-medium-3">Withdraw Saldo</h4>
        ...
        <div class="card-content ">
          <div class="card-body">
            ...
            <p></p>
            <center><button type="submit" name="wd" class="btn btn-raised btn-success btn-min-width mr-1 mb-1">Selesaikan Withdraw</button></center>
            <p></p>
            </form>
            <p class="text-muted font-small-3">Penarikan saldo yang sukses paling lambat akan ditransfer H+3 di hari kerja</p>
          </div>
        </div>
      </div>
    </div>
